...
Book::Book(const int bookID, const std::string& bookName, int pageCount,
           const std::string& authorFirstName, const std::string& authorLastName,
           const std::string& bookType)
    : bookID(bookID), bookName(bookName), pageCount(pageCount),
...
void Book::displayBookInfo() const {
    ...
    std::cout << "Title: " << bookName << "\n";
    std::cout << "Author: " << authorFirstName << " " << authorLastName << "\n";
    std::cout << "Page Count: " << pageCount << "\n";
    std::cout << "Type: " << bookType << "\n";
}
    ...
    while (std::getline(inFile, line)) {
        ...
        if (tokens.size() == 6) {
            try {
                ...
                // Handle the error (e.g., set default values or skip this entry)
            }
        }
    }
